...
export function ImageCard({
    ...
    return (
        <div
            className="flex flex-col group tactical-card p-3 rounded-[20px] animate-in fade-in fill-mode-both relative"
        ...
        >
            <Link href={link} className="block">
                ...
                <div className="px-1 mb-2">
                    <h4 className="text-[11px] font-tactical font-bold uppercase tracking-tight truncate text-white">
                        {title}
                    </h4>
                </div>
            </Link>

            <div className="px-1 flex justify-between items-center text-white mt-auto">
                <Link
                    href={`/artist/${encodeURIComponent(artist)}`}
                    className="text-[8px] font-tactical opacity-30 hover:opacity-100 transition-opacity uppercase"
                >
                    @{artist}
                </Link>
